...
| `/admin-panel/schedule` | ScheduleScreen | Загрузка / очистка / ручной ввод расписания, ручное добавление курсов и групп |
...
| `/admin-panel/profile` | ProfileScreen | Профиль текущего пользователя (имя, фамилия, должность, email, пароль) |
...
На экране пользователей (`/admin-panel/users`) в форме создания/редактирования доступны поля: имя, фамилия, должность, email.
...
- на экране входа добавлена бренд-иконка (favicon/login badge) и кнопка показа/скрытия введенного пароля
...
- экран `/admin-panel/unified-window` теперь загружает обращения из тех же записей, которые создаются во фронтенде MyKHSU-web, и работает через актуальные методы `/adminapi/unified-window/*`
